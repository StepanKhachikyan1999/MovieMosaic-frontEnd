...
function NavBar(): any {
    ...
    return (
        <>
                ...
                {/*            <form onSubmit={handleSearch} className="w-full text-sm bg-dryGray rounded flex gap-4">*/}
                {/*                <button type="submit" className="bg-subMain w-12 flex justify-center items-center h-12 rounded text-white">*/}
                {/*                    <FaSearch />*/}
                {/*                </button>*/}
                {/*                <input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t('searchMovieName')} className="font-medium placeholder:text-border text-sm w-11/12 h-12 bg-transparent border-none px-2 text-black" />*/}
                {/*            </form>*/}
                {/*        </div>*/}
                {/*        /!* Menus *!/*/}
                {/*        <div className="col-span-3 font-medium text-sm hidden xl:gap-14 2xl:gap-20 justify-between lg:flex xl:justify-end items-center">*/}
                {/*            <NavLink to="/movies" className="hover:text-subMain transitions text-white">{t('movies')}</NavLink>*/}
                {/*            <NavLink to="/about-us" className="hover:text-subMain transitions text-white">{t('about')}</NavLink>*/}
                {/*            <NavLink to="/contact-us" className="hover:text-subMain transitions text-white">{t('contact')}</NavLink>*/}
                {/*            <NavLink to={userInfo?.isAdmin ? "/dashboard" : userInfo ? "/profile" : "/login"} className="hover:text-subMain transitions text-white">*/}
                {/*                {userInfo ? (*/}
                {/*                    <img src={userInfo?.image ? userInfo?.image : 'userImage'} alt={userInfo?.fullName} className="w-8 h-8 rounded-full border object-cover border-subMain" />*/}
                {/*                ) : (*/}
                ...
                {/*                )}*/}
                {/*            </NavLink>*/}
                {/*            <NavLink to="/favorites" className="relative hover:text-subMain transitions text-white">*/}
                {/*                <FaHeart className="w-6 h-6" />*/}
                {/*                <div className="w-5 h-5 flex justify-center items-center rounded-full text-xs bg-subMain text-white absolute -top-5 -right-1">*/}
                {/*                    {likedMovies?.length || 0}*/}
                {/*                </div>*/}
